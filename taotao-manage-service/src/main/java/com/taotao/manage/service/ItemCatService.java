...
public class ItemCatService extends BaseService<ItemCat> {
    ...
    public ItemCatResult queryAllToTree() {
        ...
        try {
            // 先从缓存服务器中命中
            String str = this.redisService.get(REDIS_KEY);
            if (StringUtils.isNotEmpty(str)) {
                // 命中， 将string序列化为指定类型返回
                return MAPPER.readValue(str, ItemCatResult.class);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        try {
            ...
            // 存入缓存服务器
            this.redisService.set(REDIS_KEY, value, REDIS_TIME);
        } catch (Exception e) {
            e.printStackTrace();
        }
